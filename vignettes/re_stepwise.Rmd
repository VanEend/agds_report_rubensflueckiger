...
## Step 1: linear regression with just 1 dependent variable
...
for(i in 1:length(col_names)){
  if(i != 14){ #Is column = modelling parameter column? If not continue
    ...
    #Rounding the R^2 value
    r2 <- round(r_2, digits = 5)
    
    #Plot
    print(data %>%
      ggplot(aes(x = data[, i], y = GPP_NT_VUT_REF)) + #Points
      ...
      ggtitle(paste0("GGP_NT_VUT vs. ",col_names[i],"; R^2 = ",r2)) + #Title & R^2 value
      theme_classic())
  }
}
...
Inside this loop, the step above gets repeated. Additionally the program has to check whether the j-th column already has been used to model the GPP. If the statement is false, the program can continue. Since plotting the result has more than two dimensions and cannot be performed, it is neglected in the process below
...
ggplot(data = data, aes(x = PPFD_IN, y = GPP_NT_VUT_REF)) + #Points
  ...
  ggtitle(paste0("GGP_NT_VUT vs. PPDF_IN; R^2 = ", r_value)) + #Title & R^2 value
  theme_classic()
